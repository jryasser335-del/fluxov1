...
const LEAGUE_CATEGORIES: LeagueCategory[] = [
  ...
  // ... Puedes añadir el resto de categorías aquí según tu código original
];
...
const fetchMovieBiteId = async (homeTeam: string): Promise<string | null> => {
  try {
    ...
    // Buscamos el slug del equipo (ej: "villarreal")
    ...
    // Regex para capturar el ID completo que termina en números
...
export function AdminEvents() {
  ...
  const handleMagicSearch = async () => {
    ...
    if (realId) {
      const links = generateEmbedLinks(realId, ""); // Usamos el ID directo
  ...
  // Memo Filters (Basado en tu código original)
  const stats = useMemo(
    () => ({
        ...
        <div className="space-y-3">
          {filteredEvents.map((event, i) => (
            <EventRow
              ...
              onToggleLive={(l) => updateStreamUrls(event, { is_live: l })}
              onToggleActive={(a) => updateStreamUrls(event, { is_active: a })}
              onDelete={async () => {
...
// Componentes Auxiliares (Simplificados para no dar error)
